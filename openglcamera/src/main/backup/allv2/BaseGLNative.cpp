...
Java_sen_com_openglcamera_natives_BaseGLNative_onChangeShapeSize(JNIEnv *env, jclass type,
    ...
    if (size > 0) {
        //先减少 ，后变大
        mBaseSences->changeShapeSize( float(size) / float(max));
    }
}
...
Java_sen_com_openglcamera_natives_BaseGLNative_addTextEffect(JNIEnv *env, jclass type,
    ...
    if(bitmap){
        float width,height;
        GLuint newTextId =  createTexture2dFromBitmap(env,width,height,bitmap);
        LOGE("BaseGLNative_addTextEffect");
        mBaseSences->addTextEffect(newTextId);
        LOGE("BaseGLNative_addTextEffect2");
    }
...
JNIEXPORT void JNICALL
Java_sen_com_openglcamera_natives_BaseGLNative_onDrawFrameBefore(JNIEnv *env, jclass type,
                                                                 jobject currentData) {
