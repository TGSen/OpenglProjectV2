 ...
 * On     : 2017/11/13 14:10
 * Des    : 从新整理一下c++架构
 *          人脸识别，这个方式效率很低，每识别一帧要100多ms,考虑使用第三方的
 */
//
...
JNIEXPORT void JNICALL Java_sen_com_openglcamera_natives_BaseGLNative_onSurfaceCreated
    ...
    //开始追踪
    if(access("/sdcard/lbpcascade_frontalface.xml",R_OK)!=-1){
        //文件有可读权限
        faceDetection = new FaceDetection("/sdcard/lbpcascade_frontalface.xml");
    }
...
JNIEXPORT void JNICALL Java_sen_com_openglcamera_natives_BaseGLNative_onDrawFrame
    ...
    if(data){
        ...
        if(faceDetection){
            LOGE("sen*****");
            faceDetection->detectionFace(cameraData,width,height,eyes);
            LOGE("sen*****1");
            float endtime = getTime()-time;
            LOGE("check face usetime %f",endtime);
            mBaseSences->onDrawFrame(width,height, eyes);
